...
class Sankey {
  ...
  makeGraph(){
    ...
    this.graph.nodes = this.graph.nodes.length !== 1 ? 
      this.graph.nodes
        .sort((a,b)=>{
          return this.data.class.filter(c => c==b).length - this.data.class.filter(c => c==a).length 
        })
      : this.graph.nodes
    ...
    Object.keys(this.FS).forEach((fs)=>{
      ...
      let target = [...new Set(this.FS[fs].map(i=>this.data.class[i]))]
      ...
      target.forEach((t)=>{
        this.graph.links
          .push({
            ...
            value: this.FS[fs].map(i => this.data.class[i]).filter(c => c==t).length
          })
      })
    })
